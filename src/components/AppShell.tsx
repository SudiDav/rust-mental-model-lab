...
export function AppShell({ route, worlds, lessons, progress, onOpenLesson, onBack }: Props) {
  ...
  return <div className="min-h-screen bg-ink text-slate-100"><SkipLink /><TopBar /><main id="main-content">{route.kind === 'lesson' && lesson ? <LessonWorkspace lesson={lesson} onBack={onBack} /> : <WorldMap worlds={worlds} lessons={lessons} progress={progress} onOpenLesson={onOpenLesson} focusWorldId={route.kind === 'world' ? route.worldId : undefined} />}</main></div>;
}
